...
import { userTableColumn } from "./AllUserColumn";
import CreateUser from "./CreateUser";

type TUserManagementProps = {
  users: TUser[];
  meta: TPagination;
};

const UserAndRole = ({ users, meta }: TUserManagementProps) => {
  const [search, setSearch] = useState("");
  const searchParams = useSearchParams();
  const filteredUser = users.filter((user) => user?.role !== "SUPER_ADMIN");
  const {
    handleChange,
    show,
    setShow,
      ...
      <Card className=" w-full rounded-2xl effect p-2 gap-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[#B1B1B1] text-sm">{meta?.total} total users</p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <SelectContent>
                ...
                {["SUPER_ADMIN", "ADMIN", "AGENT"].map((role) => (
                  <SelectItem
                    key={role}
                    value={role.toString()}
